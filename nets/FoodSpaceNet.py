...
from src.utils_recipe import load_torch_model, load_model_state
...
class FoodSpaceTextEncoder(nn.Module):
    def __init__(self, opts):
        super(FoodSpaceTextEncoder, self).__init__()
    ...
    def forward(self, input, return_text_attention=None):
        # recipe embedding
        recipe_emb, attention = self.textMLP(input, return_attention=return_text_attention)
        if type(recipe_emb) is tuple: # we are assuming that if this is a tuple it also contains attention of tokens
            alpha = recipe_emb[1]
            recipe_emb = recipe_emb[0]
        recipe_emb = self.recipe_embedding(recipe_emb)
